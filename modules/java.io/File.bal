...
import qhana_backend.java.net as javanet;
import qhana_backend.java.nio.file as javaniofile;
...
public distinct class File {
    ...
    *javalang:Object;
    ...
    #
    # + return - The `javaniofile:Path` value returning from the Java mapping.
    public function toPath() returns javaniofile:Path {
        handle externalObj = java_io_File_toPath(self.jObj);
        javaniofile:Path newObj = new (externalObj);
        return newObj;
    }
    ...
    public function toURI() returns javanet:URI {
    ...
    #
    # + return - The `javanet:URL` or the `javanet:MalformedURLException` value returning from the Java mapping.
    public function toURL() returns javanet:URL|javanet:MalformedURLException {
        handle|error externalObj = java_io_File_toURL(self.jObj);
        if (externalObj is error) {
            javanet:MalformedURLException e = error javanet:MalformedURLException(javanet:MALFORMEDURLEXCEPTION, externalObj, message = externalObj.message());
            return e;
        } else {
            javanet:URL newObj = new (externalObj);
            return newObj;
        }
    }
...
# + return - The new `File` class generated.
public function newFile4(javanet:URI arg0) returns File {
